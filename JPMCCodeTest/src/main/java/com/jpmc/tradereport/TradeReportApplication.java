...
import com.jpmc.tradereport.service.TradeReportService;

public class TradeReportApplication {

	private static TradeReportService tradeReportService;

	private static final Logger logger = LoggerFactory.getLogger(TradeReportApplication.class);

	public static void main(final String[] args) throws ApplicationException {
		logger.info("Trade Report Application...");
		try {
			...
			if (cmd.hasOption("ISR")) {
				...
				tradeReportService.printDailyTradeAmounts(TradeType.SELL);
			} else if (cmd.hasOption("OSR")) {
				// Generate outgoing settlement report
				tradeReportService.printDailyTradeAmounts(TradeType.BUY);
			} else if (cmd.hasOption("IRR")) {
				// Generate incoming rankings report
				tradeReportService.printEntitiesRanking(TradeType.SELL);
			} else if (cmd.hasOption("ORR")) {
				// Generate outgoing rankings report
				tradeReportService.printEntitiesRanking(TradeType.BUY);
			} else {
